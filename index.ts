import { BlockTag, TransactionResponse, ethers } from "ethers";
...
const isFirstBuy = (txData: TransactionResponse) => {
...
(async () => {
    ...
    provider.on("block", async (blockNumber: BlockTag) => {
        ...
        let blockTxs = blockData?.prefetchedTransactions.filter((tx: TransactionResponse) => (isFirstBuy(tx))) || [];
